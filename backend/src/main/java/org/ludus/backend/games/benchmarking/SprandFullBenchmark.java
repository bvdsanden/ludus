...
/**
 * Run the algorithms on Sprand graphs with both the integer algorithm, as well
 * as the double algorithm using two different values of epsilon.
 *
...
public class SprandFullBenchmark extends Benchmark {
    ...
    public void run(Integer numberOfIterations,
        ...
        file.printf(
                ...
                "N", "edgeRatio", "maxWeight1", "maxWeight2",
                "PolicyIterationN", "EnergyGameN", "ZwickPatersonN",
                "PolicyIterationRE105", "EnergyGameRE105", "ZwickPatersonRE105",
                "PolicyIterationRInt", "EnergyGameRInt", "ZwickPatersonRInt");
        System.out.printf(
                "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n",
                "N", "edgeRatio", "maxWeight1", "maxWeight2",
                "PolicyIterationN", "EnergyGameN", "ZwickPatersonN",
    ...
    private void runAlgorithmsSprand(Integer numberOfVertices, Integer edgeRatio,
        ...
        // Solve using double algorithms, epsilon 10E-5.
        ...
        if (runEG) {
            long egResult = runEG(sprandDoubleGraph, epsilon);
            egSecD2 = ((egResult) / 1000000000.0f);
        }
        float zpSecD2 = 0.0f;
        if (runZP) {
            long zpResult = runZP(sprandDoubleGraph, epsilon);
            zpSecD2 = ((zpResult) / 1000000000.0f);
        }

        file.printf("%d,%d,%d,%d,%f,%f,%f,%f,%f,%f,%f,%f,%f\n",
                numberOfVertices, edgeRatio, maxWeight1, maxWeight2,
                piSec, egSec, zpSec,
                piSecD, egSecD, zpSecD,
                piSecD2, egSecD2, zpSecD2);
