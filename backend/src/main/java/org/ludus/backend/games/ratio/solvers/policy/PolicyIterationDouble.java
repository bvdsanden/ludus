...
import static org.ludus.backend.games.algorithms.DoubleFunctions.*;
...
public class PolicyIterationDouble {
    ...
    final static Logger logger = LoggerFactory.getLogger(PolicyIterationDouble.class);
    ...
    public static <V, E> Tuple<Map<V, Double>, StrategyVector<V, E>> solve(
            RatioGamePolicyIteration<V, E, Double> game) {
     ...
     */
    public static <V, E> Tuple<Map<V, Double>, StrategyVector<V, E>> solve(
            RatioGamePolicyIteration<V, E, Double> game, Double epsilon) {
     ...
     */
    public static <V, E> Tuple<Map<V, Double>, StrategyVector<V, E>> solve(
            RatioGamePolicyIteration<V, E, Double> game, StrategyVector<V,E> initialStrategy, Double epsilon) {
        if (GraphChecks.checkEachNodeHasSuccessor(game)) {
            return policyIteration(game, initialStrategy, epsilon);
        } else {
            System.out.println("Input game graph is not valid. "
    ...
    private static <V, E> Tuple<Map<V, Double>, StrategyVector<V, E>> policyIteration(
        ...
        while (improvement) {
            ...
            Quadruple<Map<V, Double>, Map<V, Double>, StrategyVector<V, E>, Map<V, Double>> result
                    = improveStrategyPlayer1(game, currentStrategy, distVector, ratioVector, dw2vector, epsilon);
            ...
            for (V v : game.getV0()) {
                for (E e : game.outgoingEdgesOf(v)) {
                    ...
                    Double w2sum_v = dw2vector.get(v);
                    Double w2sum_u = dw2vector.get(u) + w2;

                    Double delta = Math.max(w2sum_v, w2sum_u) * epsilon;

                    Double dv = distVector.get(v);
                    Double du = distVector.get(u) + reweighted;

                    // Player 0 wants to maximize the ratio.
                    if (lessThan(ratioVector.get(v), ratioVector.get(u), epsilon)
                            || (equalTo(ratioVector.get(v), ratioVector.get(u), epsilon)
        ...
        while (improvement) {
            ...
            for (V v : game.getV1()) {
                for (E e : game.outgoingEdgesOf(v)) {
                    ...
                    Double w2sum_v = dw2_i_t.get(v);
                    Double w2sum_u = dw2_i_t.get(u) + w2;
                    ...
                    if (greaterThan(r_i_t.get(v), r_i_t.get(u), epsilon)
                            || (equalTo(r_i_t.get(v), r_i_t.get(u), epsilon)
     ...
     */
    protected static <V, E> Tuple<Set<V>, Map<V, Double>> FindCyclesInRestrictedGraph(
    ...
    private static <V, E> Triple<Map<V, Double>, Map<V, Double>, Map<V, Double>> ComputeDistances(
        ...
        for (V u : selectedVertices) {
            if (equalTo(r_i_t.get(u), r_prev.get(u), epsilon)) {
