 ...
 * both strategies are optimal. Both players may use multiple iterations within
 * one round to get an optimal counter strategy.
 * <p>
 ...
 */
public class PolicyIterationIntManyMany {

    private final static Logger logger = LoggerFactory.getLogger(PolicyIterationIntManyMany.class);
    ...
    public static <V, E> Tuple<Map<V, Fraction>, StrategyVector<V, E>> solve(RatioGamePolicyIteration<V, E, Integer> game) {
        ...
        return solve(game, currentStrategy);
    }
     ...
     * optimal strategy.
     */
    public static <V, E> Tuple<Map<V, Fraction>, StrategyVector<V, E>> solve(RatioGamePolicyIteration<V, E, Integer> game, StrategyVector<V, E> vector) {
        ...
        StrategyVector<V, E> currentStrategy = vector;
        ...
        while (globalImprovement) {
            ...
            Quadruple<Boolean, Map<V, Fraction>, Map<V, Fraction>, StrategyVector<V, E>> result1
                    = improveStrategyPlayer1(game, currentStrategy, distVector, ratioVector);
            distVector = result1.getMiddleLeft();
            ratioVector = result1.getMiddleRight();
            currentStrategy = result1.getRight();
            ...
            // Improve the strategy of player 0.
            Quadruple<Boolean, Map<V, Fraction>, Map<V, Fraction>, StrategyVector<V, E>> result0
                    = improveStrategyPlayer0(game, currentStrategy, distVector, ratioVector);
            distVector = result0.getMiddleLeft();
            ratioVector = result0.getMiddleRight();
            currentStrategy = result0.getRight();

            logger.debug("Vectors after stategy improvement player 0: ");
            logger.debug("Distance vector: {}", printVector(game, distVector));
            logger.debug("Ratio vector: {}", printVector(game, ratioVector));

            // Next iteration if at least one of them improved.
            globalImprovement = result1.getLeft() || result0.getLeft();
        }

        return Tuple.of(ratioVector, currentStrategy);
    }

    private static <V, E> Quadruple<Boolean, Map<V, Fraction>, Map<V, Fraction>, StrategyVector<V, E>>
    improveStrategyPlayer0(RatioGamePolicyIteration<V, E, Integer> game, StrategyVector<V, E> currentStrategy,
        ...
        while (improvement) {
            ...
            for (V v : game.getV0()) {
                for (E e : game.outgoingEdgesOf(v)) {
                    ...
                    if (isSmaller(r_i_t.get(v), r_i_t.get(u))
                            || (r_i_t.get(v).equals(r_i_t.get(u))
                            && isSmaller(d_i_t.get(v), (d_i_t.get(u).add(reweighted))))) {
                        // Improve strategy.
        ...
        return Quadruple.of(change, d_i_t, r_i_t, s_i_t);
    }
     ...
     * @param game            out-degree-one graph to be evaluated
     * @param currentStrategy current strategy of both players
     * @return selected vertex and ratio of each cycle
     */
    private static <V, E> Tuple<Set<V>, Map<V, Fraction>> FindCyclesInRestrictedGraph(
