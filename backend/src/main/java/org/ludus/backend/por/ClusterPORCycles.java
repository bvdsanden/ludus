...
/**
 * Cluster partial-order reduction that preserves nonblockingness by checking cycles,
 * and expanding at least one state fully on each cycle.
 *
 * @author Bram van der Sanden
 */
public class ClusterPORCycles {
    ...
    private void dfsTraversal(List<Location> initialState) {
        ...
        while(!stack.isEmpty()) {
            ...
            // Look at all outgoing events in the cluster-inspired ample set.
            Set<String> enabled = FSMComposition.enabled(fsmList, state);

            Queue<String> queueAmple = new LinkedList<>();
            Set<String> cample = cample(fsmList, state);
            queueAmple.addAll(cample);
            boolean handledBackEdge = false;
            ...
            while(!queueAmple.isEmpty()) {
                ...
                if (!isVisited(e)) {
                    ...
                    if (!isVisited(getLocation(targetState))) {
                        ...
                        // This is a back edge, add all events not in cample to the stack as well
                        // but only if we did not do this yet.
                        if(!handledBackEdge) {
                            Set<String> toProcess = new HashSet<>(enabled);
                            toProcess.removeAll(cample);
                            queueAmple.addAll(toProcess);
                            handledBackEdge = true;
                        }
                    }
                }
            }
    ...
    private Set<String> cample(List<FSM<Location, Edge>> fsmList, List<Location> state) {
        ...
        for (String event : enabled) {
            if(!FSMComposition.isInvisible(fsmList,state,event)) {
                // We found an event that is visible, so we need to return the set of all enabled events.
                return enabled;
            }
        }
